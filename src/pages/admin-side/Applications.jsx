...
function Applications() {
  ...
  const handleDelete = async (deleteId) => {
    try {
      ...
      if (response.status === 200) {
        setApplication(application.filter((agent) => agent._id !== deleteId));
      }
    } catch (error) {
        ...
        <tbody>
          {application.map((agent) => (
            <tr key={agent._id}>
              ...
              <td className="AField">
                <button onClick={() => handleDelete(agent._id)}>Reject</button>
              </td>
              <td className="AField">
                <button onClick={() => handleAccept(agent._id)}>Accept</button>
              </td>
            </tr>
          ))}
        </tbody>
